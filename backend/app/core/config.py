...
class Settings(BaseSettings):
    ...
    DENSE_EMBEDDING_MODEL: str = (
        "sentence-transformers/all-MiniLM-L6-v2"  # 默认密集嵌入模型
    )
    ...
    DATA_ROOT_PATH="/Users/envys/aidata"
    @model_validator(mode="after")
    def validate_data_root_path(self) -> Self:
        if not self.DATA_ROOT_PATH:
            raise ValueError("DATA_ROOT_PATH must be set in environment variables")
        return self
